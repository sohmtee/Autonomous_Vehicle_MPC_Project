...
Cf = 30000;   Cr = 30000; %NOTE: UNITS
l1 = 1.2;     l2 = 1.22; %NOTE: CHANGE
l  = l1 + l2; %NOTE: CHANGE
m  = 1280; %NOTE: UNITS
Iz = 2500; %NOTE: UNITS

delta_t = 0.05; %NOTE: UNITS
Np      = 35; %NOTE: UNITS

Q_psi_kin = 1000;  Q_y_kin = 400;  R_du_kin = 500;
Q_psi_dyn = 1000;  Q_y_dyn = 400;  R_du_dyn = 2000;

du_max = deg2rad(20); %Max Rate of Change of Steering Angle
u_max  = deg2rad(30); %Max Steering Angle

v_switch_lo = 4.0; %Low Threshold for Hysteresis (Dynamic to Kinematic)
v_switch_hi = 5.0; %High Threshold for Hysteresis (Kinematic to Dynamic)

vx_min_dyn = 0.5; %Minimum Dynamic Velocity
vx_min_kin = 0; %Minimum Kinematic Velocity

%% Load reference trajectory, NOTE: CHANGE TO YOUR PATH
traj = readmatrix('C:\Users\yousi\OneDrive\Desktop\SomtoPaper\Sedan3_3.csv');
time     = traj(:,1);
X_path   = traj(:,2);
Y_path   = traj(:,3);
psi_path = traj(:,5);
...
% v_ref2 = interp1(time, speed_path, t_uniform, 'pchip');
v_ref = interp1(time, speed_path, t_uniform, 'pchip');
...
% Reference steering feedforward NOTE: Can we get steering angle from
% RoadRunner?
...
Cd_dyn = [0 1 0 0; 
          0 0 0 1];
%NOTE: This is where I decided to call it a night 4/20-21/2026 12:50am

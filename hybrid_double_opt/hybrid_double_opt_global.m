% True LTV-MPC — 2-Input [vx, steer] Kinematic/Dynamic Switch
...
for k = 1 : sim_steps
    ...
    if strcmp(active_model, 'kinematic')
        e_global = [x_kin(1) - X_ref(k); 
                    x_kin(2) - Y_ref(k); 
                    angdiff(psi_ref(k), x_kin(3))];
                    
        C_mat = [ cos(psi_ref(k)), sin(psi_ref(k)), 0;
                  0,               0,               1;
                 -sin(psi_ref(k)), cos(psi_ref(k)), 0];
                 
        y_out = C_mat * e_global;
        dx = x_kin - x_kin_prev;
        x_a = [dx; y_out];
        Q_bar = Q_kin_bar;
        R_bar = R_kin_bar;
        ...
        e_global = [x_dyn(5) - X_ref(k);
                    x_dyn(4) - Y_ref(k);
                    angdiff(psi_ref(k), x_dyn(2))];
                    
        cp = cos(psi_ref(k));  sp = sin(psi_ref(k));
        y_out = [ e_global(1)*cp + e_global(2)*sp;
                  e_global(3);
                 -e_global(1)*sp + e_global(2)*cp ];
    ...
    for i = 1:Np
        ...
        % Reference-shift correction (Crucial for augmented Absolute Models)
        idx_next = min(idx + 1, N_total);
        psi_r_next = psi_ref(idx_next);
        cp_n = cos(psi_r_next);  sp_n = sin(psi_r_next);
        
        dX_ref   = X_ref(idx_next)   - X_ref(idx);
        dY_ref   = Y_ref(idx_next)   - Y_ref(idx);
        dpsi_ref = angdiff(psi_ref(idx), psi_r_next);

        d_seq{i} = -[ cp_n*dX_ref + sp_n*dY_ref;
                      dpsi_ref;
                     -sp_n*dX_ref + cp_n*dY_ref ];
        ...
        if strcmp(active_model, 'kinematic')
            ...
            f_r = [v_r * cos(psi_r + beta_r);
                   v_r * sin(psi_r + beta_r);
                   (v_r / l2) * sin(beta_r)];
    ...
    %% Build LTV Prediction Matrices (W, Z, D_horizon)
    n_a = n_st + p;
    W = zeros(p * Np, n_a);
    Z = zeros(p * Np, 2 * Np);
    D_horizon = zeros(p * Np, 1);    
    ...
    for i = 1:Np
        ...
        for j = 1:i
            ...
            % Accumulate reference shift
            d_tilde = [zeros(n_st, 1); d_seq{j}];
            D_horizon((i-1)*p+1 : i*p) = D_horizon((i-1)*p+1 : i*p) + C_a * temp_prod * d_tilde;
        end
    end
    ...
    if strcmp(active_model, 'kinematic')
        ...
        ode_kin  = @(~, s) [vx_kin * cos(s(3) + beta_now);
                            vx_kin * sin(s(3) + beta_now);
                            (vx_kin / l2) * sin(beta_now)];
...
grid on;
legend('Reference','Kinematic', 'Dynamic', 'FontName','Times New Roman' ,'FontSize',15, 'Location','best');

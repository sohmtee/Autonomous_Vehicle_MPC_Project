...
% Dimensions
nn = 5;   % state:  [v_y, e_psi, psi_dot, e_Y, e_X]   <-- GLOBAL pose errors
...
% psi_ref grid: uniform over [-pi, pi]. The dynamics depend on psi_ref only
% through cos / sin, which are 2*pi periodic, so this covers all headings.
% Step of pi/18 (=10 deg) is a good compromise between accuracy and #LMIs.
psi_step  = pi/18;
...
% Synthesis nominal values for v_y_ref and psi_dot_ref. These enter the
% Jacobian off-diagonal entries (B_v's df1/dvx and df3/dvx) and A's rows
% 4-5 entries. Zero is a fine nominal for synthesis; the runtime code
% can use actual values when running its own model.
psi_dot_nom = 0;
v_y_nom     = 0;
...
% =========================================================================
% Runtime-side helper (drop-in replacement for interpGain).
% Add to hybrid_sparse_double_opt.m and use:
%   L_now = interpGain2D(vx_now, psi_ref_now, vx_grid, psi_grid, L_schedule);
%   M_now = interpGain2D(vx_now, psi_ref_now, vx_grid, psi_grid, M_schedule);
% =========================================================================
%
% function K = interpGain2D(vx_q, psi_q, vx_grid, psi_grid, K_schedule)
%     % Wrap psi to [-pi, pi]
%     psi_q = atan2(sin(psi_q), cos(psi_q));
%     % Clamp vx to grid range
%     vx_q  = min(max(vx_q, vx_grid(1)), vx_grid(end));
%     % Nearest-neighbor lookup on both axes (matches the synthesis grid)
...
%% =========================================================================
%% Vehicle model: returns continuous-time (A, B, C) for the GLOBAL-POSE
%% error state at operating point (v_x, psi_ref).
%%
%% State:  [v_y, e_psi, psi_dot, e_Y, e_X]
%% Input:  [v_x, delta_f]
%% Output: [e_x_path; e_psi; e_y_path]  (path-frame, via rotation by psi_ref)
%% =========================================================================
function [A_c, B_c, C_c] = computeVehicleModel(vx, psi_ref, params, ...
    ...
    % --- A matrix ---
    %
    % Rows 1-3 (lateral dynamics): same as path-frame formulation.
    % Rows 4-5 (position-error kinematics) linearize:
    %   dY/dt = v_x*sin(psi) + v_y*cos(psi)
    %   dX/dt = v_x*cos(psi) - v_y*sin(psi)
    % at (psi = psi_ref, v_y = v_y_ref), giving:
    %   A(4,1) = cos(psi_ref)                              (∂(dY)/∂v_y)
    %   A(4,2) = v_x*cos(psi_ref) - v_y_ref*sin(psi_ref)   (∂(dY)/∂e_psi)
    %   A(5,1) = -sin(psi_ref)                             (∂(dX)/∂v_y)
    %   A(5,2) = -v_x*sin(psi_ref) - v_y_ref*cos(psi_ref)  (∂(dX)/∂e_psi)
    % At psi_ref = 0 this reduces to A(4,:) = [1, vx, 0, 0, 0] and
    % A(5,:) = [0, -v_y_ref, 0, 0, 0], matching the path-frame model
    % (with v_y_ref = 0).
    A_c = zeros(5, 5);
    ...
    % --- B matrix ---
    %
    % v_x column: ∂f/∂v_x at the operating point.
    %   df1/dvx and df3/dvx are the lateral-dynamics Jacobians (same as
    %   path-frame). For position-error rows:
    %     ∂(dY)/∂v_x = sin(psi_ref)
    %     ∂(dX)/∂v_x = cos(psi_ref)
    %   At psi_ref = 0, these reduce to [0; 1], matching the path-frame B.
    ...
    % --- C matrix ---
    %
    % Path-frame output: [e_x_path; e_psi; e_y_path] with
    %   e_x_path =  e_X*cos(psi_ref) + e_Y*sin(psi_ref)
    %   e_y_path = -e_X*sin(psi_ref) + e_Y*cos(psi_ref)
    % State order is [v_y, e_psi, psi_dot, e_Y, e_X], so columns 4 and 5
    % carry the rotation entries.
